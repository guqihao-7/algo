...
public class P3369 {
    ...
    static class AVL<T> {
        ...
        public Node findkth(int x) {
            Node u = root;
            while (u != null) {
                if (x <= getLeftSize(u))
                    u = u.left;
                else if (x <= u.cnt + getLeftSize(u))
                    return u;
                else {
                    x -= (getLeftSize(u) + u.cnt);
                    u = u.right;
                }
            }
            return null;
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (n-- > 0) {
            line = br.readLine();
            a = line.split(" ");
            int _case = Integer.parseInt(a[0]);
            ...
            if (_case == 1)
                ...
                System.out.println(avl.findRank(val));
            else if (_case == 4)
                System.out.println(avl.findkth(val).val);
            else if (_case == 5)
                System.out.println(avl.lessEntry(val).val);
            else if (_case == 6)
                System.out.println(avl.greaterEntry(val).val);
        }
    }
}
